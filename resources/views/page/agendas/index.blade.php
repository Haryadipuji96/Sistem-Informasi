<x-app-layout>
    <div class="max-w-5xl mx-auto px-4 py-10">
        <h1 class="text-4xl font-extrabold text-black text-center mb-6 flex items-center justify-center gap-3">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-black animate-pulse" viewBox="0 0 20 20" fill="currentColor">
                <path d="M6 2a1 1 0 00-1 1v1H5a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V6a2 2 0 00-2-2h-.001V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zM5 8h10v8H5V8z" />
            </svg>
            Agenda Desa
        </h1>
        ...
        <div class="mb-6 text-right">
            <a href="{{ route('agendas.create') }}" class="inline-block bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">
                ➕ Tambah Agenda Baru
            </a>
        </div>
        @endcan
                ...
                <tbody>
                    @forelse ($agendas as $agenda)
                        <tr class="border-t">
                            ...
                            <td class="px-4 py-2">
                                @can('role-A')
                                <button onclick="openEditModal({{ $agenda->id }}, '{{ $agenda->title }}', '{{ $agenda->event_date }}', '{{ $agenda->status }}')" class="text-sm bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600">Edit</button>

                                <form action="{{ route('agendas.destroy', $agenda->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')" class="inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-4 text-center text-gray-500">Belum ada agenda yang ditambahkan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
